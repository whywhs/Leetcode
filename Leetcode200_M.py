...
class Solution(object):
        ...
        for i in range(len_x):
            for j in range(len_y):
                ...
                if (i,j) in dict_list or num==0:
                    continue
                dict_list[(i,j)]=1
                list_f = self.find(i,j,len_x,len_y,dict_list)
                result = self.judge(list_f,grid,dict_list)
                while(result!=[]):
                    result_all = []
                    for k in result:
                        list_f = self.find(k[0],k[1],len_x,len_y,dict_list)
                        result = self.judge(list_f,grid,dict_list)
    ...
    def find(self,i,j,len_i,len_j,dict_list):
        result_all =[]
        i_u = i-1
        i_d = i+1
        j_r = j+1
    ...
    def judge(self,list_f,grid,dict_list):
        result=[]
        for i in list_f:
